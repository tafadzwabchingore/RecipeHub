...
import { getRecipeDetails, updateRecipe, uploadRecipeImage, upsertRecipeDetails } from '@/lib/recipes'
...
export default function EditRecipePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadRecipe() {
      const [{ data: recipeData }, recipeDetails] = await Promise.all([
        supabase
          .from('recipes')
          .select('*')
          .eq('id', id)
      ...
      if (recipeData) setRecipe(recipeData as Recipe)
      if (recipeDetails) {
