...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const externalRatingMap = new Map<number, number>()
      ratings?.forEach((row: { recipe_id: number | null; source: string | null; external_id: number | null; rating: number }) => {
        ...
        if (row.external_id && row.source === 'spoonacular') externalRatingMap.set(row.external_id, row.rating)
      })
      setRatingMap(ratingMap)
      setExternalRatingMap(externalRatingMap)
  ...
  return (
    <div className="p-6">
      ...
      {recipes.length === 0 && spoonacularFavorites.length === 0 && <p>No recipes yet.</p>}
